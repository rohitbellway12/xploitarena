      ...
      <div className="w-full max-w-md space-y-8 p-10 bg-[hsl(var(--bg-card))] rounded-3xl shadow-2xl border border-[hsl(var(--border-subtle))] backdrop-blur-xl">
        ...
        <form className="mt-8 space-y-5" onSubmit={handleSubmit(onSubmit)}>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[10px] font-black text-[hsl(var(--text-muted))] uppercase tracking-widest mb-1.5 ml-1">First Name</label>
              <input
                {...register('firstName')}
                placeholder="John"
              ...
              />
              {errors.firstName && <p className="mt-1.5 text-[10px] text-rose-500 font-bold uppercase tracking-tight ml-1">{errors.firstName.message}</p>}
            </div>
            <div>
              <label className="block text-[10px] font-black text-[hsl(var(--text-muted))] uppercase tracking-widest mb-1.5 ml-1">Last Name</label>
              <input
                {...register('lastName')}
                placeholder="Doe"
                className="block w-full rounded-2xl border border-[hsl(var(--border-subtle))] bg-[hsl(var(--text-main))]/[0.03] px-4 py-3.5 text-[hsl(var(--text-main))] placeholder-[hsl(var(--text-muted))]/40 focus:border-indigo-500/50 focus:bg-[hsl(var(--text-main))]/[0.06] sm:text-sm transition-all outline-none"
              />
              {errors.lastName && <p className="mt-1.5 text-[10px] text-rose-500 font-bold uppercase tracking-tight ml-1">{errors.lastName.message}</p>}
            </div>
          </div>
          <div>
        ...
        <p className="text-center text-[11px] text-[hsl(var(--text-muted))] font-bold uppercase tracking-widest">
          Already a legend? <a href="/login" className="text-indigo-400 hover:text-indigo-300 transition-colors underline decoration-2 underline-offset-4">Log in</a>
        </p>
      </div>
    </div>
  );
